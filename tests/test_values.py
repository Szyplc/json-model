...
from json_model.values import (simplest, bounds, optionals, branches, violations,
                               ...
                               _outranking, _names, _PREDEF_NAMES, _NO_NAME_PREDEFS, _mistyped, _ultimate, _ROOT_TYPES, _TYPE_VIOLATIONS,
                               _sites, _defs, _document, _EXTRA_NAMES,
                               _mandatory, _object_sites, _measured, _breaks,
                               _violations_constrained, _justified, _recheck)
...
def test_any_float_model_gives_a_negative_float():
    v = simplest(-1.0)
    assert v < 0.0 and type(v) is float
...
def test_a_definition_used_twice_yields_a_vector_per_use():
...
def test_an_unbranched_missing_property_breaks_the_document(model):
    ...
    for mpath, vpath, frames, node, disjunction, _ in _object_sites(sites):
...
@pytest.mark.parametrize("key", ["/^a/X", "/[/", "/a", "$Ref", "$"])
...
def test_the_type_oracle_never_contradicts_the_compiler_at_a_site(model):
    ...
    jm, compiled = _compile(model, False, None, "", False)
    defs = _defs(jm)
    for mpath, vpath, frames, props, disjunction, _ in _sites(compiled, [], [], [], jm, frozenset()):
...
def test_cli_keeps_marked_values(tmp_path, capsys):
    path = _model_file(tmp_path, {"^": [{"|": [{"a": "$INT"}, {"a": "$NULL"}]}, {"a": "$NULL"}]})
    assert jmc_script(["--auto-values", path]) == 0
    out = json.loads(capsys.readouterr().out)
    assert "# .'^'[1] branch BAD" in out
    assert "# .'^'[0].'|'[0].a invalid SKIPPED: valid for the model" in out
...
def test_the_violation_api_claims_nothing_no_oracle_proves():
    ...
    assert ".@.$E.'|'[0].$map.no-such-prop extra" in violations(KEY_MODEL)
    assert ".@.$E.'|'[0] invalid" not in violations(KEY_MODEL)
...
def test_a_conjunction_does_not_branch():
    ...
    for op in ("&", "|", "^"):
        ...
        jm, compiled = _compile(model, False, None, "", False)
        found[op] = [d for _, _, _, _, d, _ in _sites(compiled, [], [], [], jm, frozenset())]
...
def test_a_site_carries_the_outermost_disjunction():
    ...
    jm, compiled = _compile(model, False, None, "", False)
    found = [d for _, _, _, _, d, _ in _sites(compiled, [], [], [], jm, frozenset())
             if d is not None]
...
REFUSES_CASES = [
    ...
    ("$UUID", "abc", True), ("$Ref", None, False), ("$EMAIL", "not an address", True),
...
def test_an_unclaimed_extra_property_needs_no_compiler():
    ...
    assert unrooted(vectors({"?a": 0})) == [
        "# . simplest", [True, {}], "# .?a present", [True, {"a": 0}],
        "# .?a invalid", [False, {"a": None}],
        "# .no-such-prop extra", [False, {"no-such-prop": None}],
        "# .{} root SKIPPED: valid for the model"]
...
def test_a_bound_in_a_definition():
    assert bounds({"$": {"S": {"@": "", ">=": 3}}, "!a": "$S"}) == {".!a.$S.'>='": {"a": "aaa"}}
...
MARKED_MODEL = {"^": [{"|": [{"a": "$INT"}, {"a": "$NULL"}]}, {"a": "$NULL"}]}
...
def test_cli_renumbers_the_errors_file_given_by_option(tmp_path):
    path = _values_file(tmp_path, json.dumps([[False, "x"], [True, GOOD_VALUE]]))
    errors = tmp_path / "other.errors.json"
    errors.write_text(json.dumps({"values": {"py": [0, 1]}}))
    _auto(tmp_path, "--values", path, "--errors", str(errors))
    assert json.loads(errors.read_text()) == {"values": {"py": [0]}, "auto": {"py": [0]}}
    assert not (tmp_path / "m.errors.json").exists()

def test_cli_rejects_the_errors_option_without_a_values_file(tmp_path):
    assert jmc_script(["--auto-values", "--errors", str(tmp_path / "m.errors.json"),
                       _model_file(tmp_path, MARKED_MODEL)]) == 1

def test_cli_warns_about_a_missing_errors_file_given_by_option(tmp_path, caplog):
    path = _values_file(tmp_path, json.dumps([[True, GOOD_VALUE]]))
    with caplog.at_level(logging.WARNING):
        _auto(tmp_path, "--values", path, "--errors", str(tmp_path / "no.errors.json"))
    assert any("no such errors file" in r.message for r in caplog.records)

def test_cli_generates_the_vectors_beside_an_errors_file_without_an_auto_member(tmp_path):
    _errors_file(tmp_path, json.dumps({"ts": True}))
    assert [t for t in _auto(tmp_path) if isinstance(t, list)]

def test_cli_generates_the_vectors_beside_an_auto_section_of_expected_errors(tmp_path):
    path = _values_file(tmp_path, json.dumps([[False, "x"]]))
    _errors_file(tmp_path, json.dumps({"auto": {"py": [0]}}))
    auto = _auto(tmp_path, "--values", path)
    assert [t for t in auto if isinstance(t, list)]
...
COMPOSED_MODEL = {"|": [{"a": "$INT"}, {"a": "$NULL"}]}

def test_vectors_confirm_a_marked_value_the_model_builds_without_a_choice():
    tests = vectors(copy.deepcopy(COMPOSED_MODEL))
    assert "# .'|'[1] branch" in tests
    assert "# .'|'[1] branch BAD" not in tests
    assert [True, {"a": None}] in tests

NESTED_CHOICE_MODEL = {"x": {"|": [{"a": "$INT"}, {"a": "$NULL"}]},
                       "?q": {"^": ["$ANY", "/^[a-z]+$/i"]}}

def test_vectors_confirm_a_value_built_beside_a_choice_it_never_enters():
    tests = vectors(copy.deepcopy(NESTED_CHOICE_MODEL))
    assert "# .x.'|'[1] branch" in tests
    assert "# .x.'|'[1] branch BAD" not in tests
    assert [True, {"x": {"a": None}}] in tests

def test_vectors_keep_a_value_the_builder_chose_under_a_xor():
    tests = vectors(copy.deepcopy(NESTED_CHOICE_MODEL))
    assert "# .?q.'^'[1] branch BAD" in tests
    assert [None, {"x": {"a": 0}, "q": "a"}] in tests

GUARDED_MODEL = {"@": {"?b": "$INT", "?c": {"|": [{"a": "$INT"}, {"a": "$NULL"}]}}, ">": 0}
REPLACED_MODEL = {"@": {"|": [1, "$STRING"]}, ">=": 10}

def test_vectors_confirm_a_value_a_constraint_above_it_still_accepts():
    tests = vectors(copy.deepcopy(GUARDED_MODEL))
    assert "# .@.?c present" in tests
    assert "# .@.?c present BAD" not in tests
    assert [True, {"c": {"a": 0}}] in tests

def test_branches_keep_a_value_which_replaces_what_a_constraint_measures():
    jm, compiled = _compile(copy.deepcopy(REPLACED_MODEL), True, None, "", False)
    marks = set()
    found = branches(compiled, jm, marks=marks)[0]
    assert found and set(found) <= marks

@pytest.mark.parametrize("predef", ["$URL", "$URI", "$EMAIL", "$HOST", "$URL_REL"])
def test_violations_prove_a_broken_predef_the_project_already_checks(predef):
    jm, compiled = _compile({"a": predef}, True, None, "", False)
    marks = set()
    found = violations(compiled, jm, marks=marks)
    assert ".a bad" in found
    assert ".a bad" not in marks

def test_bounds_prove_a_length_the_constraints_decide():
    jm, compiled = _compile({"@": "$STRING", ">=": 2}, True, None, "", False)
    marks = set()
    assert bounds(compiled, jm, marks=marks) == {".'>='": "aa"}
    assert not marks

def test_bounds_prove_a_number_the_target_still_holds():
    jm, compiled = _compile({"@": "$INT", ">=": 10}, True, None, "", False)
    marks = set()
    assert bounds(compiled, jm, marks=marks) == {".'>='": 10}
    assert not marks

def test_bounds_keep_a_number_the_target_leaves_out():
    jm, compiled = _compile({"@": "$U32", ">=": -5}, True, None, "", False)
    marks = set()
    assert bounds(compiled, jm, marks=marks) == {".'>='": -5}
    assert marks == {".'>='"}
...
RULE_CASES = [
    ...
    ("no oracle says anything", {"^": [{"|": [{"a": "$INT"}, {"a": "$NULL"}]}, {"a": "$NULL"}]},
     "# .'^'[1] branch BAD", [None, {"a": None}]),
    ("the model builds the value without a choice", {"|": [{"a": "$INT"}, {"a": "$NULL"}]},
     "# .'|'[1] branch", [True, {"a": None}]),
]
